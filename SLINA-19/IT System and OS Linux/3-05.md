...
Исходя из того, как я понял Задание 5, при запуске например top (все скриншоты ниже идут по порядку, опишу все текстом злесь), в папке proc создается папка, соответствующая его PID, в данном случае 1555. Как только мы закрываем top, в proc папка 1555 исчезает. Это про "взаимосвязь объектов".  
